...
static void decode(unsigned int count, unsigned char* p)
{
	...
	if(count<(unsigned int)(((length+5)<<1)+1))
	{
		log("wrong information length\r\n");
		hexError();
	}
	for(i=3;i<(unsigned int)((length+5)<<1)+1;i++)
	{
		text2Char(p+i);
		//log("%x ",p[i]);
	}
	//log("\r\n");
	for(i=0;i<(unsigned int)(length+5);i++)
	{
...
int _tmain(int argc, _TCHAR* argv[])
{
	...
	if((fopen_s(&in_file,file_name,"r"))!=0)
	{
		goto parameter_error;
	}
	...
	file_name[len-2] = 'i';
	file_name[len-1] = 'n';
	if((fopen_s(&out_file,file_name,"wb"))!=0)
	{
		goto parameter_error;
	}
	hex2Bin();
